...
int main(int argc, char **argv)
{
	...
	mkfifo("/tmp/echo_worker", S_IRUSR | S_IWUSR);
	mkfifo("/tmp/echo_main", S_IRUSR | S_IWUSR);
	...
	while(1)
	{
		...
		for(i = 0; i < 3; i++)
		{
			...
			if(pid == 0)
			{
				...
				while((readn = read(client_fd[i], buf, MAXLINE)) > 0)
				{
					...
					if((worker_fd = open("/tmp/echo_worker", O_RDWR)) == -1 )
					{
						perror("Named pipe error");
						return 0;
					}		
					if((main_fd = open("/tmp/echo_main", O_RDWR)) == -1 )
					{
						perror("Named pipe error");
						return 0;
					}

					write(worker_fd, buf, sizeof(buf));
					read(main_fd, buf, sizeof(buf));
		...
		if(pid>0) {
			...
			for(i = 0; i<3; i++){
				read(worker_fd, buf, sizeof(buf));
				buf[strlen(buf)-1] = ' ';
				strcat(result, buf);
			}
			strcat(result, "\n");
			for(i=0;i<3;i++){
				write(main_fd, result, sizeof(result));
			}
		}
	}
	return 0;
