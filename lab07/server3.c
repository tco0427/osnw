		...
		if(pid == 0){
			...
			if(pid_pnc == 0){	//생산자 프로세스
				...
				*input_check = true;
				...
				semid = semget((key_t)(count+2), 0, 0666);
				
				semop(semid, &semopen, 1);	//P()

				*input_check = false;	//client 입력 받기 이전에 흐름이 넘어가는 것을 방지

				semop(semid, &semclose, 1);	//V()

				while((*input_check) == false){}

				shmid = shmget((key_t)count, sizeof(struct data), 0666);
                                shared_memory = shmat(shmid, NULL, 0);
                                shm_buf = (struct data *)shared_memory;

                                shmid_check = shmget((key_t)(count+1), 1, 0666);
                                shared_memory_check = shmat(shmid_check, NULL, 0);
                                input_check = (bool *)shared_memory_check;
